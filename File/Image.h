...
public:
    static _M_Base<__uint8>* parse(const char* filename)
    {
        ...
        memcpy(&header->file_size, file_content + FILE_SIZE, sizeof(header->file_size));
    ...
    static void save(const char* filename, _M_Base<__uint8> *content)
    {
        ...
        char *file_content = new char[header->file_size]{0};
